...
int
wait(int *status) //lab 1 change, not sure where to return childs exit status
{
  struct proc *p;
  int havekids, pid;
  struct proc *curproc = myproc();
  ...
  for(;;){
    ...
    havekids = 0;
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->parent != curproc) // p is child process of curproc
        continue;
      havekids = 1;
      if(p->state == ZOMBIE){
        ...
        p->killed = 0;
        p->state = UNUSED;
        *status = p->status; //*status is an outside int in main that we can then view to see what the exited childs status was
        release(&ptable.lock);
        return pid;
      }
    }
...
int
waitpid(int pid2, int *status, int options) //lab 1 change, not sure where to return childs exit status
{
  struct proc *p;
  int havekids, pid;
  struct proc *curproc = myproc();
  ...
  for(;;){
    ...
    havekids = 0;
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->pid != pid2) // p is child process of curproc
        continue;
      havekids = 1;
      if(p->state == ZOMBIE){
        // Found one.
